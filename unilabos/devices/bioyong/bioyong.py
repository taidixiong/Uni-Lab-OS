...
class BioyongRPC(BaseRequest):
    ...
    def create_order(self, params: dict) -> dict:
        ...
        """
        response = self.post(
            url=f'{self.host}/api/lims/order/create-order',
            params={
                "apiKey": self.api_key,
                "requestTime": self.get_current_time_iso8601(),
                "data": params
    ...
    def order_query(self, params: dict) -> dict:
        ...
        """
        response = self.post(
            url=f'{self.host}/api/lims/order/order-query',
            params={
                "apiKey": self.api_key,
                "requestTime": self.get_current_time_iso8601(),
                "data": params
    ...
    # 查询物料类型
    def query_material_type(self, key_words: str) -> list:
        """
            描述：查询物料类型
            key_words
            物料类型的代码或名称
